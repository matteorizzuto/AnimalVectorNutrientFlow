...
#### Effects of consumer movement and recycling on Nutrient stocks and Primary Productivity

Here, we use partial derivatives of the nutrient stocks and primary productivity
with respect to the consumers' death rates in either local ecosystem 
($\frac{\partial N_i}{\partial d_i}$, $\frac{\partial PP_i}{\partial d_i}$; where $i \in [1, 2]$) and the consumers' movement rate from the donor ecosystem 1 to the 
dispersers' pool ($\frac{\partial N_i}{\partial g}$, $\frac{\partial PP_i}{\partial g}$). 

This analysis allows us to not only explore the influence of individual parameters 
related to the consumers on the local nutrient 
accumulation and primary productivity, but also to assess the relative magnitude 
of these parameters' effects.

This next code chunk first calculated the value of the partial derivatives listed
above, and then produces `ggplot2` violin graphs to use further down for creating
**Figure D.17** in **Appendix D**.

```{r partial-derivs-equal-I, echo=TRUE, tidy=TRUE}
PRED <- PRED %>% mutate(.,
                dN1_g = (a1*e1*(h1*l-I1*u1))/(a1*e1*l+(d1-d1*e1+g)*u1)^2,
                dN2_g = -((a2*e1*m*(a1*e1*l-d1*(-1+e1)*u1)*(h1*l-I1*u1))/((c+m)*((a1*e1*l+(d1-d1*e1+g)*u1)^2)*(a2*e2*l-d2*(-1+e2)*u2))),
                dN1_d1 = -((a1*(-1+e1)*e1*(h1*l-I1*u1))/((a1*e1*l+(d1-d1*e1+g)*u1)^2)),
                dN2_d1 = (a2*(-1+e1)*e1*g*m*u1*(-h1*l+I1*u1))/((c+ m)*((a1*e1*l+(d1-d1*e1+g)*u1)^2)*(a2*e2*l-d2*(-1+e2)*u2)),
                dN1_d2 = 0,
                dN2_d2 = (a2*(-1+e2)*(-e2*h2*l*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)+(a1*e1*e2*I2*l*(c+m)+e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2))/((c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)*((a2*e2*l-d2*(-1+e2)*u2)^2)),
                dN1_I1 = (a1*e1)/(a1*e1*l+(d1-d1*e1+g)*u1),
                dN2_I1 = (a2*e1*g*m*u1)/((c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)*(a2*e2*l-d2*(-1+e2)*u2)),
                dN1_I2 = 0,
                dN2_I2 = (a2*e2)/(a2*e2*l-d2*(-1+e2)*u2),
                dPP1_g = (u1*(a1*e1*(-d1*(-2+e1)*h1+2*g*h1+a1*e1*I1)*l+(((d1-d1*e1+g)^2)*h1-a1*d1*(e1^2)*I1)*u1))/(a1*e1*((a1*e1*l+(d1-d1*e1+g)*u1)^2)),
                dPP1_d1 = (u1*(a1*e1*(-2*d1*(-1+e1)*h1-(-2+e1)*g*h1+a1*e1*I1)*l+(((d1-d1*e1+g)^2)*h1+a1*(e1^2)*g*I1)*u1))/(a1*e1*((a1*e1*l+(d1-d1*e1+g)*u1)^2)),
                dPP1_d2 = 0,
                dPP2_g = -((e1*m*(a1*e1*l-d1*(-1+e1)*u1)*(h1*l-I1*u1)*u2*((d2^2)*((-1+e2)^2)*((d1-d1*e1+g)^2)*h2*((c+m)^2)*(u1^2)*u2*(h2*l-I2*u2)+(a2^2)*l*(e2*(d1*(-1+e1)-g)*h2*l*(c+m)*u1*(d1*e1*e2*I2*(c+m)*u1-e2*(d1+g)*I2*(c+m)*u1+2*e1*g*m*(h1*l-I1*u1))-((e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))^2)*u2)+(a1^2)*(e1^2)*(l^2)*((c+m)^2)*(h2*l-I2*u2)*((a2^2)*(e2^2)*I2*l+(d2^2)*((-1+e2)^2)*h2*u2-a2*d2*e2*((-2+e2)*h2*l+e2*I2*u2))+2*a1*e1*l*(c+m)*(h2*l-I2*u2)*((a2^2)*e2*l*(e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))-(d2^2)*((-1+e2)^2)*(d1*(-1+e1)-g)*h2*(c+m)*u1*u2+a2*d2*e2*((-2+e2)*(d1*(-1+e1)-g)*h2*l*(c+m)*u1+(d1*e1*e2*I2*(c+m)*u1-e2*(d1+g)*I2*(c+m)*u1+e1*g*m*(h1*l-I1*u1))*u2))+a2*d2*((-c^2)*e2*((d1-d1*e1+g)^2)*(u1^2)*(h2*l-I2*u2)*((-2+e2)*h2*l+e2*I2*u2)-2*c*e2*(d1*(-1+e1)-g)*m*u1*(h2*l-I2*u2)*((-2+e2)*(d1*(-1+e1)-g)*h2*l*u1+(e1*g*h1*l-(e1*g*I1+e2*(d1-d1*e1+g)*I2)*u1)*u2)+(m^2)*(-((-2+e2)*e2*((d1-d1*e1+g)^2)*(h2^2)*(l^2)*(u1^2))-2*e2*(d1*(-1+e1)-g)*h2*l*u1*(e1*g*h1*l-(e1*g*I1+(d1-d1*e1+g)*I2)*u1)*u2+((-e1*g*h1*l+e1*g*I1*u1+e2*(d1-d1*e1+g)*I2*u1)^2)*(u2^2)))))/(a2*(c+m)*((a1*e1*l+(d1-d1*e1+g)*u1)^2)*(a2*e2*l-d2*(-1+e2)*u2)*((e2*h2*l*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)-(a1*e1*e2*I2*l*(c+m)+e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)^2))),
                dPP2_d1 = ((-1+e1)*e1*g*m*u1*(h1*l-I1*u1)*u2*((-d2^2)*((-1+e2)^2)*((d1-d1*e1+g)^2)*h2*((c+m)^2)*(u1^2)*u2*(h2*l-I2*u2)+(a1^2)*(e1^2)*(l^2)*((c+m)^2)*(h2*l-I2*u2)*(a2*e2*(d2*(-2+e2)*h2-a2*e2*I2)*l+d2*(-d2*((-1+e2)^2)*h2+a2*(e2^2)*I2)*u2)+(a2^2)*l*(-e2*(d1*(-1+e1)-g)*h2*l*(c+m)*u1*(d1*e1*e2*I2*(c+m)*u1-e2*(d1+g)*I2*(c+m)*u1+2*e1*g*m*(h1*l-I1*u1))+((e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))^2)*u2)-2*a1*e1*l*(c+m)*(h2*l-I2*u2)*((a2^2)*e2*l*(e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))-(d2^2)*((-1+e2)^2)*(d1*(-1+e1)-g)*h2*(c+m)*u1*u2+a2*d2*e2*((-2+e2)*(d1*(-1+e1)-g)*h2*l*(c+m)*u1+(d1*e1*e2*I2*(c+m)*u1-e2*(d1+g)*I2*(c+m)*u1+e1*g*m*(h1*l-I1*u1))*u2))+a2*d2*((c^2)*e2*((d1-d1*e1+g)^2)*(u1^2)*(h2*l-I2*u2)*((-2+e2)*h2*l+e2*I2*u2)+2*c*e2*(d1*(-1+e1)-g)*m*u1*(h2*l-I2*u2)*((-2+e2)*(d1*(-1+e1)-g)*h2*l*u1+(e1*g*h1*l-(e1*g*I1+e2*(d1-d1*e1+g)*I2)*u1)*u2)+(m^2)*((-2+e2)*e2*((d1-d1*e1+g)^2)*(h2^2)*(l^2)*(u1^2)+2*e2*(d1*(-1+e1)-g)*h2*l*u1*(e1*g*h1*l-(e1*g*I1+(d1-d1*e1+g)*I2)*u1)*u2-((-e1*g*h1*l+e1*g*I1*u1+e2*(d1-d1*e1+g)*I2*u1)^2)*(u2^2)))))/(a2*(c+m)*((a1*e1*l+(d1-d1*e1+g)*u1)^2)*(a2*e2*l-d2*(-1+e2)*u2)*((e2*h2*l*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)-(a1*e1*e2*I2*l*(c+m)+e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)^2)),
                dPP2_d2 = -((u2*((-a1^2)*(e1^2)*(l^2)*((c+m)^2)*(a2*e2*(-2*d2*(-1+e2)*h2+a2*e2*I2)*l+(d2^2)*((-1+e2)^2)*h2*u2)*(h2*l-I2*u2)+2*a2*d2*(-1+e2)*e2*(d1*(-1+e1)-g)*h2*l*(c+m)*u1*((d1*(-1+e1)-g)*h2*l*(c+m)*u1+((d1+g)*I2*(c+m)*u1-e1*(d1*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)-(d2^2)*((-1+e2)^2)*(d1*(-1+e1)-g)*h2*(c+m)*u1*u2*((d1*(-1+e1)-g)*h2*l*(c+m)*u1+((d1+g)*I2*(c+m)*u1-e1*(d1*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)+(a2^2)*l*((-c^2)*(e2^2)*((d1-d1*e1+g)^2)*I2*(u1^2)*(h2*l-I2*u2)-c*e2*(d1*(-1+e1)-g)*m*u1*(h2*l*(-e1*(-2+e2)*g*h1*l+e1*(-2+e2)*g*I1*u1-2*e2*(d1-d1*e1+g)*I2*u1)+2*I2*(-e1*g*h1*l+e1*g*I1*u1+e2*(d1-d1*e1+g)*I2*u1)*u2)+(m^2)*(-e2*(d1*(-1+e1)-g)*h2*l*u1*(-e1*(-2+e2)*g*h1*l-e2*(d1+g)*I2*u1+e1*((-2+e2)*g*I1+d1*e2*I2)*u1)+((-e1*g*h1*l+e1*g*I1*u1+e2*(d1-d1*e1+g)*I2*u1)^2)*u2))+a1*e1*l*(c+m)*(2*a2*d2*(-1+e2)*e2*h2*l*(-2*(d1*(-1+e1)-g)*h2*l*(c+m)*u1+(2*d1*e1*I2*(c+m)*u1-2*(d1+g)*I2*(c+m)*u1+e1*g*m*(h1*l-I1*u1))*u2)+(d2^2)*((-1+e2)^2)*h2*u2*(2*(d1*(-1+e1)-g)*h2*l*(c+m)*u1+(2*(d1+g)*I2*(c+m)*u1-e1*(2*d1*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)+(a2^2)*e2*l*(e1*h2*l*(2*d1*e2*I2*(c+m)*u1-(-2+e2)*g*m*(h1*l-I1*u1))-2*e1*I2*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1))*u2+2*e2*(d1+g)*I2*(c+m)*u1*(-h2*l+I2*u2)))))/(a2*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)*((a2*e2*l-d2*(-1+e2)*u2)^2)*(e2*h2*l*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)-(a1*e1*e2*I2*l*(c+m)+e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2))))

dN_PRED <- PRED %>% dplyr::select(., dN1_g, dN1_d1, dN1_d2, dN2_g, dN2_d1, dN2_d2) %>%
  pivot_longer(., everything(), names_to = "PartD", values_to = "Value") %>% 
  separate(., PartD, into = c("StateV", "Var"), sep = "_") %>%
  mutate(., StateV = as.factor(StateV)) %>%
  dplyr::filter(., between(Value,
                           quantile(Value, 0.05, na.rm = T),
                           quantile(Value, 0.95, na.rm = T)),
                           .preserve = TRUE) %>%
  ggplot(., aes(x = Var, y = Value)) +
  geom_violin(aes(col = Var, fill = Var), show.legend = F, alpha = 0.1, draw_quantiles = c(0.25, 0.5, 0.75)) +
  facet_wrap(.~StateV, scales = "free") +
  ylab("Change in Nutrient stocks") +
  xlab(" ") +
  ggtitle(expression(paste("Gradient-neutral consumer movement (", I[1], "=", I[2], ")")))

dPP_PRED <- PRED %>% dplyr::select(., dPP1_g, dPP1_d1, dPP1_d2, dPP2_g, dPP2_d1, dPP2_d2) %>%
  pivot_longer(., everything(), names_to = "PartD", values_to = "Value") %>% 
  separate(., PartD, into = c("StateV", "Var"), sep = "_") %>%
  mutate(., StateV = as.factor(StateV)) %>%
  dplyr::filter(., between(Value,
...
As for the gradient-neutral consumer movement scenario, here we use partial
derivatives to explore the influence of consumer-related parameters on local 
ecosystem nutrient accumulation and primary productivity.

```{r partial-derivs-I2small, echo=TRUE, tidy=TRUE}
PREDI2 <- PREDI2 %>% mutate(.,
                dN1_g = (a1*e1*(h1*l-I1*u1))/(a1*e1*l+(d1-d1*e1+g)*u1)^2,
                dN2_g = -((a2*e1*m*(a1*e1*l-d1*(-1+e1)*u1)*(h1*l-I1*u1))/((c+m)*((a1*e1*l+(d1-d1*e1+g)*u1)^2)*(a2*e2*l-d2*(-1+e2)*u2))),
                dN1_d1 = -((a1*(-1+e1)*e1*(h1*l-I1*u1))/((a1*e1*l+(d1-d1*e1+g)*u1)^2)),
                dN2_d1 = (a2*(-1+e1)*e1*g*m*u1*(-h1*l+I1*u1))/((c+ m)*((a1*e1*l+(d1-d1*e1+g)*u1)^2)*(a2*e2*l-d2*(-1+e2)*u2)),
...
dN_PREDI2 <- PREDI2 %>% dplyr::select(., dN1_g, dN1_d1, dN1_d2, dN2_g, dN2_d1, dN2_d2) %>%
  pivot_longer(., everything(), names_to = "PartD", values_to = "Value") %>% 
  separate(., PartD, into = c("StateV", "Var"), sep = "_") %>%
  mutate(., StateV = as.factor(StateV)) %>%
  dplyr::filter(., between(Value,
  ...
  ylab("Change in Nutrient stocks") +
  xlab(" ") +
  ggtitle(expression(paste("Along-gradient consumer movement (", I[1], ">>",I[2], ")")))

dPP_PREDI2 <- PREDI2 %>% dplyr::select(., dPP1_g, dPP1_d1, dPP1_d2, dPP2_g, dPP2_d1, dPP2_d2) %>%
  pivot_longer(., everything(), names_to = "PartD", values_to = "Value") %>% 
  separate(., PartD, into = c("StateV", "Var"), sep = "_") %>%
  mutate(., StateV = as.factor(StateV)) %>%
  dplyr::filter(., between(Value,
...
Finally, for this against-gradient consumer movement scenario as well, we explore
the influence of consumer-related parameters on nutrient accumulation and primary 
productivity using partial derivatives.

```{r partial-derivs-I1small, echo=TRUE, tidy=TRUE}
PREDI1 <- PREDI1 %>% mutate(.,
                dN1_g = (a1*e1*(h1*l-I1*u1))/(a1*e1*l+(d1-d1*e1+g)*u1)^2,
                dN2_g = -((a2*e1*m*(a1*e1*l-d1*(-1+e1)*u1)*(h1*l-I1*u1))/((c+m)*((a1*e1*l+(d1-d1*e1+g)*u1)^2)*(a2*e2*l-d2*(-1+e2)*u2))),
                dN1_d1 = -((a1*(-1+e1)*e1*(h1*l-I1*u1))/((a1*e1*l+(d1-d1*e1+g)*u1)^2)),
                dN2_d1 = (a2*(-1+e1)*e1*g*m*u1*(-h1*l+I1*u1))/((c+ m)*((a1*e1*l+(d1-d1*e1+g)*u1)^2)*(a2*e2*l-d2*(-1+e2)*u2)),
...
dN_PREDI1 <- PREDI1 %>% dplyr::select(., dN1_g, dN1_d1, dN1_d2, dN2_g, dN2_d1, dN2_d2) %>%
  pivot_longer(., everything(), names_to = "PartD", values_to = "Value") %>% 
  separate(., PartD, into = c("StateV", "Var"), sep = "_") %>%
  mutate(., StateV = as.factor(StateV)) %>%
  dplyr::filter(., between(Value,
  ...
  ylab("Change in Nutrient stocks") +
  xlab(" ") +
  ggtitle(expression(paste("Against-gradient consumer movement (", I[1], "<<",I[2], ")")))

dPP_PREDI1 <- PREDI1 %>% dplyr::select(., dPP1_g, dPP1_d1, dPP1_d2, dPP2_g, dPP2_d1, dPP2_d2) %>%
  pivot_longer(., everything(), names_to = "PartD", values_to = "Value") %>% 
  separate(., PartD, into = c("StateV", "Var"), sep = "_") %>%
  mutate(., StateV = as.factor(StateV)) %>%
  dplyr::filter(., between(Value,
...
### Effects of nutrient inputs on Nutrient stocks and Primary Productivity

Here, we explore whether varying the value of the local inorganic nutrient inputs
into either the donor ecosystem 1 or the recipient ecosystem 2 influences the
nutrient stock accumulation and primary productivity or either local ecosystem.

As we did with the consumer-related parameters, we use partial derivatives to
explore these relationships (i.e., $\frac{\partial N_i}{\partial I_i}$, 
$\frac{\partial PP_i}{\partial I_i}$; where $i \in [1, 2]$). In keeping with
earlier analyses (see [Control scenario: gradient-neutral consumer movement], 
[Along-gradient consumer movement], and [Against-gradient consumer movement] ), 
we vary the values of _I~1~_ and _I~2~_ over the [2, 18] range.

As this analysis is new, vis-à-vis the varying levels of local inorganic inputs,
the following code chunk not only runs the model with varying values of _I~1~_ and 
_I~2~_, but also performs stability analyses on the parameter sets involved 
before calculating the value of the partial derivative listed above. It also 
produces violin plots capturing the results of these analyses. 

The violin plot for the change in $\frac{\partial PP_i}{\partial I_i}$ appears 
in **Appendix D** as **Figure D.18** and, for this reason, we do not show it here.

```{r partial-derivs-Irange, echo=TRUE, tidy=TRUE, fig.height=6, fig.width=8}
...
PREDIrange <-rbind(PREDIrange, data.frame(TIME=seq(0,100,1), I1 = seq(0,100,1), I2 = seq(0,100,1), 
                              l = seq(0,100,1), u1 = seq(0,100,1), u2 = seq(0,100,1), 
                              a1 = seq(0,100,1), a2 = seq(0,100,1), h1 = seq(0,100,1), 
                              h2 = seq(0,100,1), d1 = seq(0,100,1), d2 = seq(0,100,1), 
                              g = seq(0,100,1), m = seq(0,100,1), c = seq(0,100,1), 
...
# defined by the for loop above
  ...
  PREDIrange[i1,] <- c(i1, I1, I2, l, u1, u2, a1, a2, h1, h2, d1, d2, g, m, c, e1, e2,
                 # Nutrient stock in ecosystem 1
                 ((d1 - d1*e1 + g)*h1 + a1*e1*I1)/(a1*e1*l + (d1 - d1*e1 + g)*u1),
                 # Primary Producers biomass in ecosystem 1
                 (d1 + g)/(a1*e1),
...
# calculate meta-ecosystem biomass stocks for each trophic compartment,
# a.k.a., meta-ecosystem biomass
PREDIrange$Ntot <- PREDIrange$N1+PREDIrange$N2
PREDIrange$Ptot <- PREDIrange$P1+PREDIrange$P2
PREDIrange$Ctot <- PREDIrange$C1+PREDIrange$C2

# now calculate recycling flux for the local ecosystem manually to double check 
# the formulae used above are correct 
PREDIrange$FLUX_Eco1_check <- PREDIrange$FLUX_P1 + PREDIrange$FLUX_C1
PREDIrange$FLUX_Eco2_check <- PREDIrange$FLUX_P2 + PREDIrange$FLUX_C2

# and the meta-ecosystem recycling flux
PREDIrange$FLUX_Ptot <- PREDIrange$FLUX_P1 + PREDIrange$FLUX_P2
PREDIrange$FLUX_Ctot <- PREDIrange$FLUX_C1 + PREDIrange$FLUX_C2
PREDIrange$MetaEcoFlux <- PREDIrange$FLUX_Eco_1 + PREDIrange$FLUX_Eco_2 - PREDIrange$FLUX_Q

# finally, calculate the production for each compartment at meta-ecosystem level
PREDIrange <- dplyr::mutate(PREDIrange, PROD_Ptot = PROD_P1 + PROD_P2, .after = "PROD_C2")
PREDIrange <- dplyr::mutate(PREDIrange, PROD_Ctot = PROD_C1 + PROD_C2, .after = "PROD_Ptot")

# Equilibrium stocks less than or equal to 0 make no biological sense, so we 
# will flag them in the PRED dataset.
PREDIrange <- PREDIrange %>% dplyr::mutate(., biosense = ifelse(N1 > 0 & 
                                                    P1 > 0 & 
                                                    C1 > 0 & 
                                                    N2 > 0 & 
                                                    P2 > 0 & 
...
for (i in 1:nrow(PREDIrange)) {
  ...
  MathStabIrange <- rbind(MathStabIrange, data.frame(TIME=i, I1, I2, l, u1, u2, a1, a2, h1,
                                         h2, d1, d2, g, m, c, e1, e2, dQ, dN1, 
                                         dP1, dC1, dN2, dP2, dC2,
                                         EiV1 = Re(eigen(Jacob)$values[1]),
                                         EiV2 = Re(eigen(Jacob)$values[2]),
                                         EiV3 = Re(eigen(Jacob)$values[3]),
                                         ...
                                         maxEv = max(Re(base::eigen(Jacob)$values)),
                                         stable = stable,
                                         biosense = PREDIrange$biosense[i]))
}

MathStabIrange$stable <- as.factor(MathStabIrange$stable)

# separate unstable equilibria to work with later
MathStabIrangeUS <- subset(MathStabIrange, MathStabIrange$stable == "unstable")

biononsense <- subset(PREDIrange, PREDIrange$biosense == "no")

identical(as.numeric(MathStabIrangeUS[ , "TIME"]), biononsense[ , "TIME"])

PREDIrange <- left_join(PREDIrange, select(MathStabIrange, !c(I1:maxEv)), by = c("TIME", "biosense"))

PREDIrangepos <- filter(PREDIrange, PREDIrange$biosense == "yes" & PREDIrange$stable == "stable")

# sample PREDpos to only use 1000 random simulation results
PREDIrange_sample1000 <- PREDIrangepos[sample(nrow(PREDIrangepos), size = 1000), ]

PREDIrange_1k <- droplevels(PREDIrange_sample1000)

PREDIrange <- PREDIrange %>% mutate(.,
                dN1_g = (a1*e1*(h1*l-I1*u1))/(a1*e1*l+(d1-d1*e1+g)*u1)^2,
                dN2_g = -((a2*e1*m*(a1*e1*l-d1*(-1+e1)*u1)*(h1*l-I1*u1))/((c+m)*((a1*e1*l+(d1-d1*e1+g)*u1)^2)*(a2*e2*l-d2*(-1+e2)*u2))),
                dN1_d1 = -((a1*(-1+e1)*e1*(h1*l-I1*u1))/((a1*e1*l+(d1-d1*e1+g)*u1)^2)),
                dN2_d1 = (a2*(-1+e1)*e1*g*m*u1*(-h1*l+I1*u1))/((c+ m)*((a1*e1*l+(d1-d1*e1+g)*u1)^2)*(a2*e2*l-d2*(-1+e2)*u2)),
                ...
                dPP2_d1 = ((-1+e1)*e1*g*m*u1*(h1*l-I1*u1)*u2*((-d2^2)*((-1+e2)^2)*((d1-d1*e1+g)^2)*h2*((c+m)^2)*(u1^2)*u2*(h2*l-I2*u2)+(a1^2)*(e1^2)*(l^2)*((c+m)^2)*(h2*l-I2*u2)*(a2*e2*(d2*(-2+e2)*h2-a2*e2*I2)*l+d2*(-d2*((-1+e2)^2)*h2+a2*(e2^2)*I2)*u2)+(a2^2)*l*(-e2*(d1*(-1+e1)-g)*h2*l*(c+m)*u1*(d1*e1*e2*I2*(c+m)*u1-e2*(d1+g)*I2*(c+m)*u1+2*e1*g*m*(h1*l-I1*u1))+((e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))^2)*u2)-2*a1*e1*l*(c+m)*(h2*l-I2*u2)*((a2^2)*e2*l*(e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))-(d2^2)*((-1+e2)^2)*(d1*(-1+e1)-g)*h2*(c+m)*u1*u2+a2*d2*e2*((-2+e2)*(d1*(-1+e1)-g)*h2*l*(c+m)*u1+(d1*e1*e2*I2*(c+m)*u1-e2*(d1+g)*I2*(c+m)*u1+e1*g*m*(h1*l-I1*u1))*u2))+a2*d2*((c^2)*e2*((d1-d1*e1+g)^2)*(u1^2)*(h2*l-I2*u2)*((-2+e2)*h2*l+e2*I2*u2)+2*c*e2*(d1*(-1+e1)-g)*m*u1*(h2*l-I2*u2)*((-2+e2)*(d1*(-1+e1)-g)*h2*l*u1+(e1*g*h1*l-(e1*g*I1+e2*(d1-d1*e1+g)*I2)*u1)*u2)+(m^2)*((-2+e2)*e2*((d1-d1*e1+g)^2)*(h2^2)*(l^2)*(u1^2)+2*e2*(d1*(-1+e1)-g)*h2*l*u1*(e1*g*h1*l-(e1*g*I1+(d1-d1*e1+g)*I2)*u1)*u2-((-e1*g*h1*l+e1*g*I1*u1+e2*(d1-d1*e1+g)*I2*u1)^2)*(u2^2)))))/(a2*(c+m)*((a1*e1*l+(d1-d1*e1+g)*u1)^2)*(a2*e2*l-d2*(-1+e2)*u2)*((e2*h2*l*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)-(a1*e1*e2*I2*l*(c+m)+e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)^2)),
                dPP2_d2 = -((u2*((-a1^2)*(e1^2)*(l^2)*((c+m)^2)*(a2*e2*(-2*d2*(-1+e2)*h2+a2*e2*I2)*l+(d2^2)*((-1+e2)^2)*h2*u2)*(h2*l-I2*u2)+2*a2*d2*(-1+e2)*e2*(d1*(-1+e1)-g)*h2*l*(c+m)*u1*((d1*(-1+e1)-g)*h2*l*(c+m)*u1+((d1+g)*I2*(c+m)*u1-e1*(d1*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)-(d2^2)*((-1+e2)^2)*(d1*(-1+e1)-g)*h2*(c+m)*u1*u2*((d1*(-1+e1)-g)*h2*l*(c+m)*u1+((d1+g)*I2*(c+m)*u1-e1*(d1*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)+(a2^2)*l*((-c^2)*(e2^2)*((d1-d1*e1+g)^2)*I2*(u1^2)*(h2*l-I2*u2)-c*e2*(d1*(-1+e1)-g)*m*u1*(h2*l*(-e1*(-2+e2)*g*h1*l+e1*(-2+e2)*g*I1*u1-2*e2*(d1-d1*e1+g)*I2*u1)+2*I2*(-e1*g*h1*l+e1*g*I1*u1+e2*(d1-d1*e1+g)*I2*u1)*u2)+(m^2)*(-e2*(d1*(-1+e1)-g)*h2*l*u1*(-e1*(-2+e2)*g*h1*l-e2*(d1+g)*I2*u1+e1*((-2+e2)*g*I1+d1*e2*I2)*u1)+((-e1*g*h1*l+e1*g*I1*u1+e2*(d1-d1*e1+g)*I2*u1)^2)*u2))+a1*e1*l*(c+m)*(2*a2*d2*(-1+e2)*e2*h2*l*(-2*(d1*(-1+e1)-g)*h2*l*(c+m)*u1+(2*d1*e1*I2*(c+m)*u1-2*(d1+g)*I2*(c+m)*u1+e1*g*m*(h1*l-I1*u1))*u2)+(d2^2)*((-1+e2)^2)*h2*u2*(2*(d1*(-1+e1)-g)*h2*l*(c+m)*u1+(2*(d1+g)*I2*(c+m)*u1-e1*(2*d1*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)+(a2^2)*e2*l*(e1*h2*l*(2*d1*e2*I2*(c+m)*u1-(-2+e2)*g*m*(h1*l-I1*u1))-2*e1*I2*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1))*u2+2*e2*(d1+g)*I2*(c+m)*u1*(-h2*l+I2*u2)))))/(a2*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)*((a2*e2*l-d2*(-1+e2)*u2)^2)*(e2*h2*l*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)-(a1*e1*e2*I2*l*(c+m)+e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2))),
                dPP1_I1 = ((d1+g)*u1)/(a1*e1*l+(d1-d1*e1+g)*u1),
                dPP1_I2 = 0,
                dPP2_I1 = (e1*g*m*u1*u2*(-((a2*e1*g*h1*l*m+a1*e1*(d2*(-1+e2)*h2-a2*e2*I2)*l*(c+m)-d2*(-1+e2)*(d1*(-1+e1)-g)*h2*(c+m)*u1-a2*(c*e2*(d1-d1*e1+g)*I2+e1*g*I1*m+e2*(d1-d1*e1+g)*I2*m)*u1)*(a2*l-d2*u2)*(e2*h2*l*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)-(a1*e1*e2*I2*l*(c+m)+e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2))+(a2*e1*g*h1*l*m+a1*e1*(d2*(-1+e2)*h2-a2*e2*I2)*l*(c+m)-d2*(-1+e2)*(d1*(-1+e1)-g)*h2*(c+m)*u1-a2*(c*e2*(d1-d1*e1+g)*I2+e1*g*I1*m+e2*(d1-d1*e1+g)*I2*m)*u1)*u2*(d2*(d1*(-1+e1)-g)*h2*l*(c+m)*u1+a2*e1*g*l*m*(h1*l-I1*u1)+d2*(d1+g)*I2*(c+m)*u1*u2-d2*e1*(d1*I2*(c+m)*u1+g*m*(h1*l-I1*u1))*u2-a1*d2*e1*l*(c+m)*(h2*l-I2*u2))-a2*(e2*h2*l*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)-(a1*e1*e2*I2*l*(c+m)+e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)*(d2*(d1*(-1+e1)-g)*h2*l*(c+m)*u1+a2*e1*g*l*m*(h1*l-I1*u1)+d2*(d1+g)*I2*(c+m)*u1*u2-d2*e1*(d1*I2*(c+m)*u1+g*m*(h1*l-I1*u1))*u2-a1*d2*e1*l*(c+m)*(h2*l-I2*u2))))/(a2*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)*(a2*e2*l-d2*(-1+e2)*u2)*((e2*h2*l*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)-(a1*e1*e2*I2*l*(c+m)+e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)^2)),
                dPP2_I2 = (u2*((a2^2)*e1*(e2^2)*(d1*(-1+e1)-g)*g*h2*(l^2)*m*(c+m)*u1*(h1*l-I1*u1)-(d2^2)*e1*((-1+e2)^2)*(d1*(-1+e1)-g)*g*h2*m*(c+m)*u1*(-h1*l+I1*u1)*(u2^2)+(a1^2)*a2*d2*(e1^2)*(e2^2)*(l^2)*((c+m)^2)*((h2*l-I2*u2)^2)+a2*d2*((e2^2)*((d1-d1*e1+g)^2)*(h2^2)*(l^2)*((c+m)^2)*(u1^2)-2*(e2^2)*(d1*(-1+e1)-g)*h2*l*(c+m)*u1*(d1*e1*I2*(c+m)*u1-(d1+g)*I2*(c+m)*u1+e1*g*m*(h1*l-I1*u1))*u2+((e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))^2)*(u2^2))-a1*e1*l*(c+m)*(a2*(e2^2)*h2*(l^2)*(2*d2*(d1*(-1+e1)-g)*h2*(c+m)*u1+a2*e1*g*m*(h1*l-I1*u1))+2*a2*d2*(e2^2)*h2*l*(2*(d1+g)*I2*(c+m)*u1-e1*(2*d1*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2+d2*(d2*e1*((-1+e2)^2)*g*h2*m*(h1*l-I1*u1)+2*a2*e2*I2*(d1*e1*e2*I2*(c+m)*u1-e2*(d1+g)*I2*(c+m)*u1+e1*g*m*(h1*l-I1*u1)))*(u2^2))))/(a2*(a2*e2*l-d2*(-1+e2)*u2)*((e2*h2*l*(c+m)*(a1*e1*l+(d1-d1*e1+g)*u1)-(a1*e1*e2*I2*l*(c+m)+e2*(d1+g)*I2*(c+m)*u1-e1*(d1*e2*I2*(c+m)*u1+g*m*(h1*l-I1*u1)))*u2)^2)))

PREDIrange %>% dplyr::select(., dN1_I1, dN1_I2, dN2_I1, dN2_I2) %>%
  pivot_longer(., everything(), names_to = "PartD", values_to = "Value") %>% 
  separate(., PartD, into = c("StateV", "Var"), sep = "_") %>%
  mutate(., StateV = as.factor(StateV)) %>%
  dplyr::filter(., between(Value,
  ...
  ggplot(., aes(x = Var, y = Value)) +
  geom_violin(aes(col = Var, fill = Var), show.legend = F, alpha = 0.1, draw_quantiles = c(0.25, 0.5, 0.75)) +
  # coord_cartesian(ylim = c(-12,10)) +
  facet_wrap(.~StateV, scales = "free") +
  ylab("Change in Nutrient stocks") +
  xlab(" ") +
  ggtitle(expression(paste("Consumer movement in a varying environment (", I[1], ", ", I[2], " sampled over the [2,18] range.)")))

# ggsave(filename = "../Results/Nstock_Irange.svg", device = "svg", dpi = 300, width = width, height = height)

dPP_Irange <- PREDIrange %>% dplyr::select(., dPP1_I1, dPP1_I2, dPP2_I1, dPP2_I2) %>%
  pivot_longer(., everything(), names_to = "PartD", values_to = "Value") %>% 
  separate(., PartD, into = c("StateV", "Var"), sep = "_") %>%
  mutate(., StateV = as.factor(StateV)) %>%
  dplyr::filter(., between(Value,
  ...
  # coord_cartesian(ylim = c(-12,10)) +
  facet_wrap(.~StateV, scales = "free") +
  ylab("Change in Primary Productivity stocks") +
  xlab(" ") +
  ggtitle(expression(paste("Consumer movement in a varying environment (", I[1], ", ", I[2], " sampled over the [2,18] range.)")))

# ggsave(dPP_Irange, filename = "../Results/PP_Irange.svg", device = "svg", dpi = 300, width = width, height = height)
```

The following code-chunk combines the individual plots for the effects of 
consumer-related parameters on nutrient stocks accumulation and primary 
productivity in local ecosystems produced in earlier sections ([Control scenario: gradient-neutral consumer movement], [Along-gradient consumer movement], and
[Against-gradient consumer movement]) into multi-panel visual objects. The figure 
combining the results for $\frac{\partial PP_i}{\partial d_i}$ and 
$\frac{\partial PP_i}{\partial g}$ appears in **Appendix D** as **Figures D.17**
and as such we do not show it here.

```{r partial-derivs-consumer-combi, echo=TRUE,tidy=TRUE, fig.height=8, fig.width=6}
dN_PRED + dN_PREDI2 + dN_PREDI1 + plot_layout(ncol = 1, nrow = 3) + plot_annotation(tag_levels = "a", tag_prefix = "(", tag_suffix = ")")

# ggsave(filename = "../Results/NutriensPD.svg", device = "svg", dpi = 300, width = height, height = width)

dPP_SUM <- dPP_PRED + dPP_PREDI2 + dPP_PREDI1 + plot_layout(ncol = 1, nrow = 3) + plot_annotation(tag_levels = "a", tag_prefix = "(", tag_suffix = ")")

# ggsave(dPP_SUM, filename = "../Results/PP_PD.svg", device = "svg", dpi = 300, width = height, height = width)
```

### Summary of consumer and environemntal parameters effect on local nutrient stocks and primary productivity

Here, we produce tables to summarize the effects of consumer-related and 
environmental on local nutrient stocks and primary productivity. Both tables can
be found in folder `../Results/` as `.rtf` files.

```{r partial-derivs-summary-tab, echo=TRUE,tidy=TRUE}
ConsParamsSUM <- bind_rows(PRED, PREDI2, PREDI1, .id = "Scenario") %>%
  select(., Scenario, stable, dN1_g:dN2_d2, dPP1_g:dPP2_d2) %>%
  pivot_longer(., dN1_g:dPP2_d2, names_to = "PartialD", values_to = "Value") %>%
  separate(., PartialD, into = c("PartialD", "Param"), sep = "_") %>%
  mutate(., Scenario = as_factor(Scenario),
         Scenario = fct_recode(.f = Scenario, "Gradient-neutral consumer movement (I~1~ = I~2~)" = "1", "Along-gradient consumer movement (I~1~ >> I~2~)" = "2", "Against-gradient consumer movement (I~1~ << I~2~)" = "3"),
         PartialD = as_factor(PartialD),
         PartialD = fct_recode(.f = PartialD, "N^*^~1~" = "dN1", "N^*^~2~" = "dN2", "PP^*^~1~" = "dPP1", "PP^*^~2~" = "dPP2"),
         PartialD = as.character(PartialD)) %>%
  filter(., stable == "stable") %>%
  group_by(., Param, Scenario, PartialD) %>%
  dplyr::filter(., between(Value,
                           quantile(Value, 0.05, na.rm = T),
                           quantile(Value, 0.95, na.rm = T)),
                           .preserve = TRUE) %>%
  summarise(., min = min(Value), 
            ...
            .groups = "keep") %>% 
  dplyr::ungroup() %>% 
  pivot_wider(., names_from = Param, values_from = c(min, median, max)) %>% 
  gt(., groupname_col = "Scenario", rowname_col = "PartialD", process_md = T) %>% 
  tab_spanner(label = md("**Consumer movement rate from ecosystem 1 (g)**"), columns = c("min_g", "median_g", "max_g")) %>%
  tab_spanner(label = md("**Consumer recycling rate in ecosystem 1 (d~1~)**"), columns = c("min_d1", "median_d1", "max_d1")) %>%
  tab_spanner(label = md("**Consumer recycling rate in ecosystem 2 (d~2~)**"), columns = c("min_d2", "median_d2", "max_d2")) %>%
  cols_label(PartialD = md("**Compartment**"), "min_g" = md("*Min.*"), "median_g" = md("*Median*"), "max_g" = md("*Max.*"), "min_d1" = md("*Min.*"), "median_d1" = md("*Median*"), "max_d1" = md("*Max.*"), "min_d2" = md("*Min.*"), "median_d2" = md("*Median*"), "max_d2" = md("*Max.*")) %>%
  fmt_number(., columns = c(3:11), decimals = 2) %>%
  fmt_markdown(., columns = 2) %>%
  tab_footnote(
    footnote = md("N^*^~i~ stands for Nutrient stock in ecosystem _i_ = [1, 2]"),
    locations = cells_row_groups()) %>%
  tab_footnote(
    footnote = md("PP^*^~i~ stands for Primary Productivity in ecosystem _i_ = [1, 2]"),
    locations = cells_row_groups()) %>%
  gtsave(., filename = "../Results/ConsParamsSUM.rtf")

PREDIrange %>%
  select(., stable, dN1_I1:dN2_I2, dPP1_I1:dPP2_I2) %>%
  pivot_longer(., dN1_I1:dPP2_I2, names_to = "PartialD", values_to = "Value") %>%
  separate(., PartialD, into = c("PartialD", "Param"), sep = "_") %>%
  mutate(., PartialD = as_factor(PartialD),
         PartialD = fct_recode(.f = PartialD, "N^*^~1~" = "dN1", "N^*^~2~" = "dN2", "PP^*^~1~" = "dPP1", "PP^*^~2~" = "dPP2"),
         PartialD = as.character(PartialD)) %>%
  filter(., stable == "stable") %>%
  group_by(., Param,, PartialD) %>%
  dplyr::filter(., between(Value,
                           quantile(Value, 0.05, na.rm = T),
                           quantile(Value, 0.95, na.rm = T)),
                           .preserve = TRUE) %>%
  ...
  pivot_wider(., names_from = Param, values_from = c(min, median, max)) %>% 
  gt() %>% 
  tab_spanner(label = md("**Inorganic inputs into ecosystem 1 (I~1~)**"), columns = c("min_I1", "median_I1", "max_I1")) %>%
   tab_spanner(label = md("**Inorganic inputs into ecosystem 2 (I~2~)**"), columns = c("min_I2", "median_I2", "max_I2")) %>%
  cols_label(PartialD = md("**Compartment**"), "min_I1" = md("*Min.*"), "median_I1" = md("*Median*"), "max_I1" = md("*Max.*"), "min_I2" = md("*Min.*"), "median_I2" = md("*Median*"), "max_I2" = md("*Max.*")) %>%
  ...
  tab_footnote(
    footnote = md("PP^*^~i~ stands for Primary Productivity in ecosystem _i_ = [1, 2]"),
    locations = cells_row_groups()) %>%
  gtsave(., filename = "../Results/InorgInputsSUM.rtf")
```
